...
개발에 필요한 직접적인 코드는 안드로이드 스튜디오에서 작성하지만 버전 관리를 위해 Git을 사용하는데 SourceTree 또는 커맨드명령을 매번 왔다갔다 하면서 사용하는것은 매우 번거로운 일이다. 안드로이드 스튜디오에서 자체적으로 Git을 포함한 다양한 VCS(Version Control System)와 연동할 수 있는 기능을 제공하는데, 이 포스트에서는 Git과 연동하는 방법에 대해서 설명한다.
